...
npm test           # 90 pure-Node checks (no window, no network)
...
npm run test:ui    # 71-step real-window playtest (real engines): search,
                    ...
                    #   via the one-click presets) — applies it and
                    #   switches/clears it from the tray header), Settings
                    #   night mode (weekday presets), a one-click night-pill
                    #   session override, chip tooltips that break down own
                    #   limit vs plan window vs night cap, a tray warning
                    #   when the plan window and night mode overlap at
                    #   different caps, per-source save folder, and a real
