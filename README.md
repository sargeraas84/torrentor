...
npm test           # 77 pure-Node checks (no window, no network)
...
npm run test:resume # boots the real app TWICE over a slow Range server: starts
                    ...
                    #   second scenario proving a paused transfer parks across
                    #   a restart and per-source folders survive
npm run test:ui    # drives the real window (real engines): search, favorites,
                    #   VPN check, paging, thumbnails, paced demo (paused &
                    #   resumed, queue reordered by drag-and-drop), per-source
                    #   save folder, and a real Archive direct download — needs network
