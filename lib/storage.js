...
const DEFAULTS = {
  prefs: {
    engines: {
      ...
      // Third-party community indexes are opt-in on fresh installs.
      piratebay: false,
      nyaa: false,
      yts: false,
      '1337x': false,
    ...
    // Optional per-source default folders for direct downloads, keyed by
    // engine id (Settings → Library). A source with no entry falls back to
    // the shared last-used folder above. demo-curated/archive-org/
    // distro-releases/arch-releases expose direct files.
    downloadDirs: {},
    // Bandwidth-aware queue scheduling: when on, queued downloads start in
    // estimated-finish order (smallest remaining first) instead of FIFO.
    smartOrder: false,
  },
  history: [],
  favorites: [],
  ...
  transfers: [],
  // Lifetime per-source download tallies ({ count, bytes } keyed by engine
  // id / 'other') shown on the Library views.
  stats: {},
};
...
class Storage {
  ...
  _load(key, fallback) {
    try {
      ...
      if (key === 'stats') return parsed && typeof parsed === 'object' && !Array.isArray(parsed) ? parsed : JSON.parse(JSON.stringify(fallback));
  ...
  /**
   * Replace a whole prefs key, bypassing the nested-object merge. The
   * merge keeps keys that exist on the stored side, so it silently
   * resurrects deleted entries inside collections like queuePlans — a
   * replaced value is the only way to persist a removal.
   */
  replacePrefs(key, value) {
  ...
  /** Replace the persisted per-source download tallies. */
  setStats(obj) {
    this.stats = obj && typeof obj === 'object' && !Array.isArray(obj) ? obj : {};
    this._schedule('stats');
    return this.stats;
  }

  /** Immediate flush (used on quit / in tests). */
  flush() {
    for (const key of ['prefs', 'history', 'favorites', 'health', 'transfers', 'stats']) this._write(key);
  }
}
