 ...
 * unlimited) additionally capped by the applied plan's schedule when the
 * clock is inside the window. This is what stream pacing honors per chunk.
 */
function effectiveLimitBps(t) {
  const own = Math.max(0, Math.floor(Number(t && t.maxBytesPerSec) || 0));
  const cap = scheduleCapNow();
  if (cap <= 0) return own;
  return own > 0 ? Math.min(own, cap) : cap;
}
...
function etaDetail(t) {
  ...
  if (limit > 0) {
    rate = limit;
    // The schedule window's cap is the binding rate when it caps this
    // file below (or instead of) its own per-file limit — say so on the
    // chip instead of claiming the file chose that speed.
    basis = activePlanSchedule && scheduleCapNow() > 0 && (Math.max(0, Math.floor(Number(t.maxBytesPerSec) || 0)) === 0 || scheduleCapNow() < Math.max(0, Math.floor(Number(t.maxBytesPerSec) || 0))) ? 'window' : 'limit';
  } else if (t.rateBps > 0) {
    rate = t.rateBps;
    basis = 'measured';
