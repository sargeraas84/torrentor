...
/**
 * Normalize a what-if patch ({ transferId: bytesPerSec }) into stable
 * plan entries [{ filePath, bytesPerSec }]. Unknown ids are dropped, so a
 * patch can outlive the session it was built for.
 */
function planEntries(patch) {
  const out = [];
  for (const [id, bps] of Object.entries(patch || {})) {
    const t = transfers.get(Number(id));
    if (t && t.filePath) {
      out.push({ filePath: t.filePath, bytesPerSec: Math.max(0, Math.floor(Number(bps) || 0)) });
    }
  }
  return out;
}
...
/**
 * Apply saved plan entries to the current transfers (matched by
 * destination path), setting each listed file's speed limit for real and
 * re-sorting a smart-ordered queue. Returns how many limits were applied.
 */
function applyPlanEntries(entries, onEvent) {
  let applied = 0;
  for (const e of entries || []) {
    if (!e || !e.filePath) continue;
    const t = [...transfers.values()].find((x) => x.filePath === e.filePath);
    if (!t || (t.status !== 'queued' && t.status !== 'downloading' && t.status !== 'paused')) continue;
    t.maxBytesPerSec = Math.max(0, Math.floor(Number(e.bytesPerSec) || 0));
