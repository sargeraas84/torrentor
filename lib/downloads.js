...
async function runTransfer(id, onEvent) {
  ...
  try {
    ...
    if (cancelled && t.status === 'paused') {
      // User paused mid-stream: the .part is kept and the slot freed, but
      // the transfer stays parked (resumableSnapshot never lists it, so it
      // does NOT auto-resume on next launch — the user resumes it manually).
      t.error = null;
      t.speedBytesPerSec = 0;
      t.finishedAt = Date.now();
