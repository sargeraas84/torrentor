 ...
 * this, so the queue and the UI always agree.
 */
function etaDetail(t) {
  if (!t) return { etaSeconds: null, rateBps: 0, basis: 'size-unknown' };
  const limit = Math.max(0, Math.floor(Number(t.maxBytesPerSec) || 0));
  if (t.total == null || !(t.total > 0)) return { etaSeconds: null, rateBps: limit || 0, basis: 'size-unknown' };
  ...
  return { etaSeconds: remaining / rate, rateBps: rate, basis };
}
...
function snapshot() {
  ...
  return [...running, ...queued, ...paused, ...rest].slice(0, TRANSFER_LIMIT).map((t) => {
    ...
    // does: the estimated time to finish and the speed behind it.
    if (smartOrder && t.status === 'queued') {
      const d = etaDetail(t);
