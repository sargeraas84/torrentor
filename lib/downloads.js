...
let activePlanName = '';
let activePlanSchedule = null; // { from: 'HH:MM', to: 'HH:MM', bytesPerSec, days? } | null
// Session-only FORCE on the armed plan's schedule window (like the night
// pill's override): null = follow the clock window, true = window active
// regardless of the clock, false = window inactive regardless. Resets on
// every arm/clear and on the next boot.
let activePlanForce = null;
// Provenance of the CURRENT arm: restored = the plan was re-armed from
// prefs at boot (not applied this session); appliedAt = ISO timestamp of
// the last explicit apply (persisted with the arm).
let activePlanRestored = false;
let activePlanAppliedAt = null;
...
let globalSchedule = null; // { from: 'HH:MM', to: 'HH:MM', bytesPerSec, days? } | null
// Session-only override of night mode (the tray pill click): null = follow
// the configured clock window, true = force the cap on regardless of the
// window, false = force it off. NOT persisted — it resets on the next boot.
...
/**
 * Normalize a weekday selector (array of 0–6, JS Date.getDay() convention:
 * 0 = Sunday … 6 = Saturday) to a sorted unique array, or null when absent
 * / empty / invalid (null = every day of the week).
 */
function normalizeDays(days) {
  ...
  for (const d of days) {
    const n = Number(d);
    if (Number.isInteger(n) && n >= 0 && n <= 6) set[n] = true;
  }
 ...
 * An optional `dayOfWeek` (0–6) restricts the window to specific weekdays
 * when the schedule carries a non-empty `days` array.
 */
function scheduleWindowActive(schedule, minute, dayOfWeek) {
  ...
  if (days && (dayOfWeek == null || days.indexOf(Number(dayOfWeek)) < 0)) return false;
...
/** Current local weekday (0–6, Date.getDay()). */
function nowDay() {
  return new Date().getDay();
}

/** Public shape of a schedule: the `days` key only appears when set. */
function scheduleShape(s) {
  if (!s) return null;
  const out = { from: s.from, to: s.to, bytesPerSec: s.bytesPerSec };
  const days = normalizeDays(s.days);
...
function scheduleCapNow() {
  ...
  if (!s) return 0;
  if (activePlanForce === false) return 0;
  if (activePlanForce === true) return s.bytesPerSec;
  if (!scheduleWindowActive(s, nowMinute(), nowDay())) return 0;
  return s.bytesPerSec;
}

/**
 * Force for the ARMED plan's window (the per-plan sibling of the night
 * pill's override). Pass true to make the armed plan's schedule cap apply
 * regardless of the clock, false to suppress it regardless of the clock,
 * or null to follow the window again. As of v1.4.9 this force is the
 * plan's own persisted toggle — main writes it into the plan record + the
 * applied-plan pref and re-applies it at boot — so only re-arming the
 * plan or clearing it resets the runtime state here.
 */
function setPlanForce(force) {
  activePlanForce = force === true ? true : force === false ? false : null;
  if (smartOrder && waiting.length > 1) sortQueue(); // cap changes ETAs → re-rank
  return appliedPlanInfo();
}

/**
 * Record HOW the current arm happened, for the tray's provenance note:
 * restored=true means boot re-armed it from prefs (appliedAt is the last
 * explicit apply time); restored=false means the user just applied it.
 */
function noteAppliedPlan({ restored, appliedAt } = {}) {
  if (!arguments.length) {
    activePlanRestored = false;
    activePlanAppliedAt = null;
    return true;
  }
  activePlanRestored = !!restored;
  activePlanAppliedAt = appliedAt || null;
  return true;
}
...
function setGlobalSchedule(schedule) {
  ...
  globalSchedule = from != null && to != null && bps > 0 ? { from: s.from, to: s.to, bytesPerSec: bps, days: normalizeDays(s.days) } : null;
  if (smartOrder && waiting.length > 1) sortQueue(); // cap changes ETAs → re-rank
  return globalScheduleInfo();
}

/**
 * Session-only night-mode override (the tray pill click). Pass true to
 * force the night cap on regardless of the clock window, false to force it
 * off, or null to go back to following the window. Returns the new state.
 */
function setNightOverride(on) {
  nightOverride = on === true ? true : on === false ? false : null;
  if (smartOrder && waiting.length > 1) sortQueue();
  return globalScheduleInfo();
}

/** Night-mode cap binding right now (0 = inactive / not set / overridden off). */
function globalCapNow() {
  const s = globalSchedule;
  if (!s) return 0;
  if (nightOverride === true) return s.bytesPerSec;
  if (nightOverride === false) return 0;
  if (!scheduleWindowActive(s, nowMinute(), nowDay())) return 0;
  return s.bytesPerSec;
}
...
function globalScheduleInfo() {
  ...
  return {
    schedule: scheduleShape(globalSchedule),
    windowActive: capNow > 0,
    capNow,
    // Session override: null (follow window), true (forced on), false (forced off).
...
function setActivePlan(name, schedule) {
  ...
  const to = s ? parseClock(s.to) : null;
  const bps = s ? Math.max(0, Math.floor(Number(s.bytesPerSec) || 0)) : 0;
  activePlanSchedule = from != null && to != null && bps > 0 ? { from: s.from, to: s.to, bytesPerSec: bps, days: normalizeDays(s.days) } : null;
  // A fresh arm follows the clock until the user forces it, and its
  // provenance starts blank — main calls noteAppliedPlan right after to say
  // whether this was an explicit apply or a boot restore.
  activePlanForce = null;
  activePlanRestored = false;
  activePlanAppliedAt = null;
  if (smartOrder && waiting.length > 1) sortQueue(); // cap changes ETAs → re-rank
  return { name: activePlanName, schedule: scheduleShape(activePlanSchedule) };
}
...
function appliedPlanInfo() {
  ...
  return {
    name: activePlanName,
    schedule: scheduleShape(activePlanSchedule),
    windowActive: capNow > 0,
    capNow,
    // Session force on the armed window (null = follow the clock, true =
    // forced on, false = forced off) + how this arm happened (applied in
    // this session vs restored at boot, with the last explicit apply time).
    force: activePlanForce,
    restored: activePlanRestored,
    appliedAt: activePlanAppliedAt,
  };
}

/**
 * Per-transfer cap breakdown for the chip tooltip: the file's own limit,
 * the applied plan's window cap (when its window is active) and the night
 * cap (when active), plus the effective speed and which source binds.
 * Returns null when no window/night cap is actually binding the transfer.
 */
function capBreakdown(t) {
  const own = Math.max(0, Math.floor(Number(t && t.maxBytesPerSec) || 0)) || null;
  const pc = scheduleCapNow();
  const nc = globalCapNow();
  const planWindow = pc > 0 ? { cap: pc, name: activePlanName } : null;
  const night = nc > 0 ? { cap: nc } : null;
  const sources = [];
  if (own) sources.push(own);
  if (planWindow) sources.push(planWindow.cap);
  if (night) sources.push(night.cap);
  const effective = sources.length ? Math.min.apply(null, sources) : null;
  const binding = effective == null ? null : effective === own ? 'limit' : effective === pc ? 'window' : 'night';
  if (!planWindow && !night) return null; // no shared cap → nothing to explain
  return { own, planWindow, night, effective, binding };
}
