...
// boot and persists statsSnapshot() alongside the transfer records.
let stats = {}; // engineId -> { count, bytes }
...
function setStats(seed) {
  ...
  if (seed && typeof seed === 'object') {
    for (const [k, v] of Object.entries(seed)) {
      const count = Math.max(0, Math.floor(Number(v && v.count) || 0));
      const bytes = Math.max(0, Math.floor(Number(v && v.bytes) || 0));
      if (count > 0 || bytes > 0) stats[String(k)] = { count, bytes };
    }
  }
  return statsSnapshot();
}

/** Shallow copy of the tallies for persistence. */
function statsSnapshot() {
  const out = {};
  for (const [k, v] of Object.entries(stats)) out[k] = { count: v.count, bytes: v.bytes };
  return out;
}
...
function recordStats(url, bytes) {
  ...
  const cur = stats[engineId] || { count: 0, bytes: 0 };
  cur.count += 1;
  cur.bytes += Math.max(0, Math.floor(Number(bytes) || 0));
  stats[engineId] = cur;
}
 ...
 * automatic second life.
 */
function resumableSnapshot() {
  const out = [];
  for (const t of transfers.values()) {
    if ((t.status === 'downloading' || t.status === 'queued' || t.status === 'paused') && t.filePath) {
      out.push({
        url: t.url,
        filePath: t.filePath,
        demo: !!t.demo,
...
function restorePending(list, onEvent) {
  ...
  for (const rec of arr) {
    ...
    if (dup) continue;
    const t = makeEntry(rec.url, rec.filePath, { maxBytesPerSec: rec.maxBytesPerSec, folderRule: rec.folderRule });
