...
// Rolling bandwidth (bytes/sec) observed from live streams; smart order
// uses it as the assumed rate for unlimited transfers so its estimates
// are in seconds, not raw bytes.
let measuredBps = 0;
...
function etaScore(t) {
  ...
  const rate = t.maxBytesPerSec > 0 ? t.maxBytesPerSec : measuredBps > 0 ? measuredBps : 102400;
...
function sortQueue() {
  ...
  const scored = waiting.map((id) => ({ id, score: etaScore(transfers.get(id)) }));
  scored.sort((a, b) => (a.score !== b.score ? a.score - b.score : idx.get(a.id) - idx.get(b.id)));
  waiting = scored.map((x) => x.id);
}
...
function throttled(onEvent, t) {
  ...
  return (received, total) => {
    ...
    if (dt >= 500) {
      ...
      measuredBps = bps; // rolling bandwidth feeds smart-order estimates
      lastTick = now;
      lastBytes = received;
